...
function putOptions(id, record, mimeType, encoder) {
  return {
    method: 'PUT',
...
function wrapXml(record) {
  return `<?xml version="1.0" encoding="UTF-8"?><pharmacy>${jsontoxml(record)}</pharmacy></xml>`;
}

async function putRecordAsXml(id, record) {
  return new Promise((resolve, reject) => {
    request(putOptions(id, record, 'application/xml', wrapXml), (error, response, body) => {
...
async function putRecordAsJson(id, record) {
  return new Promise((resolve, reject) => {
    request(putOptions(id, record, 'application/json', JSON.stringify), (error, response, body) => {
      if (!error && response.statusCode === 200) {
        resolve(body);
      } else {
        reject(error);
