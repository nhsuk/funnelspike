...
function getAltOptions(day, coordinates, results) {
  const geoRange = `&maxdist=32.19&origin=${coordinates.latitude},${coordinates.longitude}&sort=prox&num_ranks=${results}`;
  const query = `altDate:${day}`;
  const url = urlStart + query + geoRange;
  console.log(url);
  return {
    method: 'get',
    url,
...
async function getAlterations(moment, coordinates, maxResults = 10) {
  const day = moment.format('YYYY-MM-DD');
  const res = await search(getAltOptions(day, coordinates, maxResults));
  const results = JSON.parse(res).response.resultPacket.results;
  return results.map(mapResult);
}
module.exports = {
