...
// function getOtSessionKeys(openingTimesAsOffset) {
//   const sessions = {};
//   // eslint-disable-next-line
//   for (const [index, value] of openingTimesAsOffset.entries()) {
//     sessions[`s${index}`] = value.session;
//   }
//   return sessions;
// }
...
function addOffsetTimes(record) {
  ...
  if (record.openingTimes) {
    ...
    // eslint-disable-next-line no-param-reassign
    // record.otSessions = getOtSessionKeys(record.openingTimesAsOffset);
    // eslint-disable-next-line no-param-reassign
